...
function updateDataChangesItem(_ref) {
  var change = _ref.change,
      dataChangesItem = _ref.dataChangesItem,
      store = _ref.store,
      deepPath = _ref.deepPath;
  var serverDataObj = store.dataServer[deepPath];
...
function createDataChangesItem(_ref2) {
  var change = _ref2.change,
      store = _ref2.store,
      deepPath = _ref2.deepPath;
  var splitPath = deepPath.split('/');
  var deepObj = splitPath.reduce(function (acc, val) {
    if (!acc[val]) {
      acc[val] = {};
    }
...
function deepPathChanged(_ref3) {
  var change = _ref3.change,
      deepPath = _ref3.deepPath,
      store = _ref3.store;
  var dataChangesItem = store.dataChanges.get(deepPath);

  if (isObject(dataChangesItem)) {
    updateDataChangesItem({
      change: change,
      deepPath: deepPath,
      store: store,
      dataChangesItem: dataChangesItem
    });
  } else {
    console.log('Create data changesItem');
    createDataChangesItem({
      change: change,
      deepPath: deepPath,
      store: store
    });
  ...
  (0, _mobx.action)(function () {
    Object.getOwnPropertyNames(store.data).forEach(function (key) {
      if ((0, _mobx.isComputedProp)(store.data, key)) {
        ...
        var disposer = (0, _mobx.observe)(store.data, key, function (_ref4) {
...
var FormStore = (_class = /*#__PURE__*/function () {
  ...
  function FormStore(options, data) {
    ...
    store.observeDataPropertiesDisposer = (0, _mobxUtils.deepObserve)(store.data, store.storeDataChanged); // store.observeDataPropertiesDisposer = observe(store.data, store.storeDataChanged);
